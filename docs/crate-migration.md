...
After `0.0.1-alpha.1` has actually been published, the future registry form is:

```toml
[dependencies]
stern = { version = "=0.0.1-alpha.1", features = ["vello-winit"] }
```
...
`=0.0.1-alpha.1` requirement. A package dry-run is not publication and is not
