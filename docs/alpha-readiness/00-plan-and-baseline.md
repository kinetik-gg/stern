...
| Token checkpoint | Documentation verification passed; Stages 1-6 subsequently completed and Stage 7 is Current / Authorized with `PERF-01` next |
...
Stage 0 completed only after all ten target documents existed, all 43 unique IDs were represented, index-local links resolved, dependencies and overlap zones matched the accepted plan, and documentation-only checks passed. Stages 1-6 subsequently completed, and Stage 7 is Current / Authorized with `PERF-01` next. A stale path, invented finding, changed alpha scope, or write outside the allowed paths remains a stop condition.
