...
fn interactive_request(output: &stern_widgets::WidgetOutput, cursor: CursorShape) -> bool {
    output
        .platform_requests
...
fn assert_disabled_not_focused(name: &str, output: &stern_widgets::WidgetOutput) {
...
fn assert_disabled_after_enabled_press<F, G, H>(
    ...
    F: FnMut(&mut UiMemory) -> stern_widgets::WidgetOutput,
    G: FnMut(&mut UiMemory) -> stern_widgets::WidgetOutput,
    H: FnMut(&mut UiMemory) -> stern_widgets::WidgetOutput,
{
...
fn checked_radio_labels(output: &stern_core::FrameOutput) -> Vec<&str> {
    output
        .semantics
        .nodes()
        .iter()
...
fn frame_slider_current(output: &stern_core::FrameOutput) -> f32 {
    output
        .semantics
        .nodes()
        .iter()
