...
fn frame_context_at(now: Duration, input: UiInput) -> stern_core::FrameContext {
    stern_core::FrameContext::new(
        ViewportInfo::new(
...
fn text_value(output: &stern_core::FrameOutput, role: &SemanticRole) -> Option<String> {
    output
        .semantics
        .nodes()
        .iter()
...
fn has_selection_highlight(output: &stern_core::FrameOutput, theme: &stern_core::Theme) -> bool {
    let selection = theme
...
fn render_text_wrapper(case: TextWrapperCase, disabled: bool) -> stern_core::FrameOutput {
