//! Production Vello translation evidence for actual focused list-row surfaces.
...
#[test]
fn actual_list_row_focus_translates_as_contained_fill_only_annuli_at_release_scales() {
    ...
    .enumerate()
    {
        let id = WidgetId::from_key(format!("vello-list-row-{case}"));
        let rect = Rect::new(origin.x, origin.y, 128.5, 24.25);
        ...
        for (scale, physical_size) in [
            (1.0_f32, PhysicalSize::new(160, 64)),
            (1.25, PhysicalSize::new(200, 80)),
            (1.5, PhysicalSize::new(240, 96)),
            (2.0, PhysicalSize::new(320, 128)),
        ] {
