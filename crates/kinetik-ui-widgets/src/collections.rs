...
pub fn virtual_range(request: VirtualRangeRequest) -> Range<usize> {
    ...
    let scroll_offset = clamp_virtual_scroll_offset(
        request.scroll_offset,
        request.item_count,
        item_extent,
        viewport_extent,
    );
    let first = (scroll_offset / item_extent).floor() as usize;
    let visible = (viewport_extent / item_extent).ceil() as usize + 1;
