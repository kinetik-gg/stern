...
mod node_graph_conformance {
    ...
    use kinetik_ui_widgets::{
        ...
        NodeGraphEdgeRoutePoint, NodeGraphEmissionError, NodeGraphFrameMove, NodeGraphGridStyle,
        NodeGraphHitTarget, NodeGraphHitTestConfig, NodeGraphHitTestError,
        NodeGraphLinkDraftCompletionError, NodeGraphLinkDraftEndpoint,
        ...
        RerouteDescriptor, RerouteId, filter_node_graph_add_node_search_entries,
    ...
    fn link_edit_graph() -> NodeGraphDescriptor {
        let number = PortTypeId::from_raw(10);
        let vector = PortTypeId::from_raw(11);
        NodeGraphDescriptor {
            nodes: vec![
    ...
    fn context_targets_preserve_hit_identity_for_all_graph_surfaces() {
        ...
        assert_eq!(
            NodeGraphContextTarget::from_hit_target(NodeGraphHitTarget::NodeTitle(
                NodeId::from_raw(10)
            )),
            NodeGraphContextTarget::Node(NodeId::from_raw(10))
        );
        assert_eq!(
            NodeGraphContextTarget::from_hit_target(NodeGraphHitTarget::NodeBody(
                NodeId::from_raw(11)
            )),
        ...
        assert_eq!(
            NodeGraphContextTarget::from_hit_target(NodeGraphHitTarget::Frame(
                NodeFrameId::from_raw(7)
            )),
            NodeGraphContextTarget::Frame(NodeFrameId::from_raw(7))
        );
        assert_eq!(
            NodeGraphContextTarget::from_hit_target(NodeGraphHitTarget::Group(
    ...
    fn context_delete_and_duplicate_requests_preserve_selected_targets() {
        ...
        let unselected = graph.context_actions(
            NodeGraphContextTarget::Port(PortEndpoint::new(
                NodeId::from_raw(1),
                PortId::from_raw(5),
            )),
            &selection,
    ...
    fn context_frame_group_and_canvas_actions_are_metadata_only() {
        ...
        let frame_selection =
            context_action(&node_actions, NodeGraphContextActionKind::FrameSelection);
    ...
    fn disabled_and_unavailable_context_actions_are_deterministic() {
        ...
        let graph = NodeGraphDescriptor {
            nodes: vec![
                ...
                NodeDescriptor::new(NodeId::from_raw(3), "Unused", GraphRect::ZERO).with_ports(
                    vec![PortDescriptor::new(
                        PortId::from_raw(3),
                        PortDirection::Output,
                        "Unused",
    ...
    fn add_node_search_selection_helpers_skip_disabled_entries() {
        ...
        let disabled = NodeGraphAddNodeSearchSelection::from_selected(
            NodeGraphAddNodeDescriptorId::from_raw(2),
        );
        ...
        let graph = NodeGraphDescriptor {
            ...
            frames: vec![
                NodeFrameDescriptor::new(
                    frame,
                    "Frame",
                    GraphRect::new(-10.0, -10.0, 140.0, 100.0),
                )
